...
from .models import ScoredCandidate, IdeaInput
...
class BrainstormerAgent(BaseAgent):
    ...
    async def execute(self, input_data: IdeaInput) -> AgentResult:
        ...
        try:
            ...
            scamper_names = await self._apply_scamper(
                input_data.idea or "", input_data.max_candidates // 3
            )

            # Apply role-storming
            role_names = await self._apply_role_storming(
                input_data.idea or "", input_data.max_candidates // 3
            )
            ...
            return AgentResult(
                agent_name=self.name, success=True, data=final_names, execution_time=execution_time
            )
...
class AnalyzerAgent(BaseAgent):
    ...
    async def _enhance_scoring(self, candidate: ScoredCandidate) -> ScoredCandidate:
        ...
        enhanced_score = (
            0.30 * original_score + 0.25 * seo_score + 0.25 * brand_safety + 0.20 * cultural_score
        )
...
class PackagerAgent(BaseAgent):
    ...
    async def execute(self, input_data: List[ScoredCandidate]) -> AgentResult:
        ...
        try:
            ...
            return AgentResult(
                agent_name=self.name, success=True, data=package_data, execution_time=execution_time
            )
    ...
    def _generate_summary(self, candidates: List[ScoredCandidate]) -> Dict[str, Any]:
        ...
        available_domains = sum(
            1 for candidate in candidates for domain in candidate.domains if domain.available
        )
...
async def run_multi_agent_research(
    idea: str, tlds: Optional[List[str]] = None, max_candidates: int = 50
) -> Dict[str, Any]:
    """
    ...
    idea_input = IdeaInput(idea=idea, tlds=tlds, max_candidates=max_candidates)
